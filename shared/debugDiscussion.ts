...
export function discussionHandoffText(request: Entity): string {
  ...
  if (request.data.textContext) return `${instruction}\n\n这是文字选区批注。textContext 保存原文、卡片 ID、字段与批注时版本。start/end 是渲染后文字偏移，不是 Markdown 源码位置；先核对最新正文及前后文，原文可能已被更新，不要直接按旧偏移替换。\n\n用户批注：${request.data.instruction}\n\n文字上下文：\n${JSON.stringify(request.data.textContext,null,2)}`;
