...
export function showcaseOperations(root:string,prefix='showcase'):Operation[] {
 ...
 for(const op of ops)if(op.op==='create'&&op.id.startsWith(id('map')+':node-')) {
  if(op.kind==='mind'){const n=Number(op.id.split('-').at(-1));op.data.title=['持久化共创','内容与关系','批注与 Agent'][n];op.data.body=['每个图节点都可以成为一个子画布。','卡片、文件、任务共享内容；布局独立。','讨论请求携带对象 ID 和版本；支持选中文字和多卡片批注。'][n];}
 }
