...
for(const file of ['dist','server','shared','public','package.json','package-lock.json','README.md','docs','start.sh','start.cmd'])cpSync(path.join(root,file),path.join(dir,file),{recursive:true});
mkdirSync(path.join(dir,'scripts'));
for(const file of ['backup.ts','stop.ts'])cpSync(path.join(root,'scripts',file),path.join(dir,'scripts',file));
const files=[];
