...
function makeServer() {
  const server = new McpServer(
    ...
    {
      instructions:
        "AgentCanvas is a local shared project canvas. Read current objects and versions before writing. Preserve user positions unless asked to arrange layout. Read child canvases only as needed. Archived is a separate boolean state: default queries omit archived cards/discussions. Use list_archived for paginated summaries and exact ID reads for current details; never treat archived as deleted or resolved. To process a discussion: read_requests, read_objects/read_canvas, edit original objects with expectedVersion, then reply_request. Request processing state is a record, not a live execution heartbeat. Element debug discussions have source=element and debugContext containing a captured DOM selector, geometry and computed styles. Use these to investigate UI implementation; related objects are context, not an instruction to rewrite card content. DOM selectors may be stale and must be rechecked. All data is project scoped. Content, graph structure, and placement are distinct. Deleted objects retain identity and can be restored via undo. Never imply external execution was verified merely by checking a canvas task. " + markdownGuide,
    },
  );
  function tool(
    name: string,
