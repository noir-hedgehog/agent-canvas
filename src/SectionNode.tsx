...
export function SectionNode({data,selected}:NodeProps<CanvasNodeType>){
 ...
 return <div className={`canvas-section ${selected?'selected':''}`}>
  <div className="section-heading" aria-label={`移动 Section：${data.entity.data.title}`}>
   ...
   {!readOnly&&<button className="nodrag nopan" aria-label="解除 Section" title="解除分组，保留全部卡片和位置" onClick={async()=>{await store.getState().settleEdits(data.entity.id);const e=store.getState().snapshot?.entities.find(e=>e.id===data.entity.id);if(e&&await store.getState().run([{op:'delete',id:e.id,expectedVersion:e.version}],'解除 Section'))store.setState({selected:[]});}}><Ungroup size={16}/></button>}
  </div>
 </div>;
}
